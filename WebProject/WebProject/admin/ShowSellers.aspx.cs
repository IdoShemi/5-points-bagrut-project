...
{
    public partial class ShowSellers : System.Web.UI.Page
    {
        ...
        protected void Selectbutton(object sender, EventArgs e)
        {
            ...
            if (InsertMail.Text.ToString() != "")
            {
                ...
                Con1.Open();
                string sqlstring = $"SELECT * FROM sellers WHERE MyEmail='{InsertMail.Text.ToString()}'";
            ...
            else
            {
                ...
                Con1.ConnectionString = @"Provider=Microsoft.ACE.OLEDB.12.0; Data source=" + Server.MapPath("") + "\\..\\database.accdb";
                Con1.Open();
                string sqlstring = "SELECT * FROM sellers";
                OleDbCommand Cmd = new OleDbCommand(sqlstring, Con1);
                OleDbDataReader dr = Cmd.ExecuteReader();
                if (dr.HasRows)
                {
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "DeleteUser")
            {
                ...
                Con1.ConnectionString = @"Provider=Microsoft.ACE.OLEDB.12.0; Data source=" + Server.MapPath("") + "\\..\\database.accdb";
                Con1.Open();
                string sqlstring = "DELETE * FROM sellers WHERE Myusername='" + user + "';";
                OleDbCommand cmd = new OleDbCommand(sqlstring, Con1);
                int y = 0;
                y = cmd.ExecuteNonQuery();
                Response.Redirect("/admin/ShowSellers.aspx");
            }
